...
class DataLoader(object):
    ...
    def __next__(self):
        def pad_to_longest(insts, max_len,pad):
            inst_data = np.array([inst + [pad] * (max_len - len(inst)) for inst in insts])
        ...
        self._step += 1
        data = pad_to_longest(self._src_sents[_start:_start+_bsz], self._max_len,0) #PAD=0
